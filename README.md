...
## Висновки: 
    1.Який формат найкраще підходить для якого типу зображень?
        PNG використовується для зображень з прозорістю та високою деталізацією. Менш ефективний для фотографій через великий розмір файлу.
        JPG підходить для фотографій та зображень із багатьма кольорами. Добре стискається без значної втрати якості.
        WEBP сучасний формат із кращим стисненням, ніж JPEG та PNG. Підтримує як прозорість, так і анімацію.
        AVIF формат із кращим стисненням, ніж WebP і JPEG.
    2.Вплив розміру зображень на оптимізацію:
        -збільшується швидкість завантаження для клієнтів
        -зменшення ваги, а отже більше місця на сервері
    3. Як правильно адаптувати зображення для Retina-дисплеїв?
        -Зробити розмір зображення в 2х або 3х рази більше, ніж область де воно буде знаходитись.
        -Використовувати низьку якість стиснення.
        -Використовувати формати WebP та AVIF, так як вони добре поєднують гарну якість зображення при його малому розмірі. 
